...
const envSchema = z.object({
  ...
  EDITOR_USER_IDS: z
    ...
    .refine((value) => value.length > 0, "EDITOR_USER_IDS must contain at least one integer")
    .refine(
      (value) => value.every((item) => /^-?\d+$/.test(item)),
      "EDITOR_USER_IDS must be a comma-separated list of integers"
    )
    .transform((value) => Array.from(new Set(value.map((item) => Number.parseInt(item, 10))))),
