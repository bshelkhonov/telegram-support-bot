...
const isBlockedByUserError = (error: GrammyError): boolean =>
  error.error_code === 403 && /bot was blocked by the user/i.test(error.description);

const extractRelayCommandText = (text: string): string | null => {
  const match = text.match(/^\/(?:r|reply)(?:@[A-Za-z0-9_]+)?(?:\s+([\s\S]+))?$/);
  if (!match) {
    return null;
  }
...
export const registerAdminReplyHandler = (
  ...
  bot.on("message", async (ctx) => {
    ...
    if (!("forward_origin" in repliedMessage) || !repliedMessage.forward_origin) {
      return;
    }
    ...
    if (ctx.msg.text) {
      ...
      if (commandText !== null) {
        if (!commandText) {
          await ctx.api.sendMessage(options.adminChatId, "Usage: /r <message>", {
    ...
    try {
      await ctx.api.copyMessage(binding.userId, options.adminChatId, ctx.msg.message_id);
    } catch (error) {
      if (error instanceof GrammyError && isBlockedByUserError(error)) {
        await ctx.api.sendMessage(
          options.adminChatId,
